    ...
    <div class="row">
        ...
        <div class="col-md-9">
            ...
            <div class="card shadow-sm">
                <div class="card-body">
                    <form method="POST" action="<?= url("/o/") ?><?= htmlspecialchars($tenant['slug']) ?>/s/<?= htmlspecialchars($sport['slug']) ?>/seasons">
                        <?= csrf_field() ?>
                        
                        <div class="mb-3">
                            <label for="name" class="form-label">Season Name</label>
                            <input type="text" class="form-control" id="name" name="name" required placeholder="e.g. 2025/2026">
                        </div>

                        <div class="row mb-3">
                            <div class="col">
                                <label for="starts_on" class="form-label">Start Date</label>
                                <input type="date" class="form-control" id="starts_on" name="starts_on" required>
                            </div>
                            <div class="col">
                                <label for="ends_on" class="form-label">End Date</label>
                                <input type="date" class="form-control" id="ends_on" name="ends_on" required>
                            </div>
                        </div>

                        <div class="mb-3 form-check">
                            <input type="checkbox" class="form-check-input" id="is_current" name="is_current" value="1">
                            <label class="form-check-label" for="is_current">Set as Current Season</label>
                            <div class="form-text">This will unset any currently active season for this sport.</div>
                        </div>

                        <button type="submit" class="btn btn-primary">Create Season</button>
                        <a href="<?= url("/o/") ?><?= htmlspecialchars($tenant['slug']) ?>/s/<?= htmlspecialchars($sport['slug']) ?>/seasons" class="btn btn-outline-secondary">Cancel</a>
                    </form>
                </div>
            </div>
        </div>
