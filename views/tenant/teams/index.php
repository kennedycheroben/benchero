        ...
        <div class="col-md-9">
            <div class="d-flex justify-content-between align-items-center mb-3">
                ...
                <?php if (in_array($role, ['owner', 'admin', 'manager'])): ?>
                    <a href="<?= url("/o/") ?><?= htmlspecialchars($tenant['slug']) ?>/s/<?= htmlspecialchars($sport['slug']) ?>/teams/create" class="btn btn-primary">+ New Team</a>
                <?php endif; ?>
            </div>
            <hr>
                ...
                <div class="list-group">
                    <?php foreach ($teams as $team): ?>
                        <div class="list-group-item d-flex justify-content-between align-items-center">
                            <div>
                                <h5 class="mb-1">
                                    <a href="<?= url("/o/") ?><?= htmlspecialchars($tenant['slug']) ?>/s/<?= htmlspecialchars($sport['slug']) ?>/teams/<?= htmlspecialchars($team['id']) ?>" class="text-decoration-none">
